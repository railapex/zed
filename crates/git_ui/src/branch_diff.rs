...
impl BranchDiff {
    pub(crate) fn register(workspace: &mut Workspace, cx: &mut Context<Workspace>) {
        workspace.register_action(|workspace, _: &DeployBranchDiff, window, cx| {
            Self::deploy_branch_diff(workspace, false, window, cx)
        });
    ...
    pub(crate) fn deploy_branch_diff(
        ...
        require_default_branch_mode: bool,
        window: &mut Window,
        ...
        let intended_repo_id = intended_repo.read(cx).id;
        ...
        window
            .spawn(cx, async move |cx| {
                ...
                workspace.update_in(cx, |workspace, window, cx| {
                    ...
                    let is_current = (!require_default_branch_mode
                        || git_store.read(cx).diff_base() == GitDiffBaseSetting::DefaultBranch)
                        && git_store
                            .read(cx)
                            .active_repository()
                            .is_some_and(|repo| repo.read(cx).id == intended_repo_id);
                    if !is_current {
                        return anyhow::Ok(());
                    }
