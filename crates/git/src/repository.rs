...
impl GitRepository for RealGitRepository {
    ...
    fn diff_tree(&self, request: DiffTreeType) -> BoxFuture<'_, Result<TreeDiff>> {
        ...
        let args = match request {
            DiffTreeType::MergeBase { base, head } => vec![
            ...
            DiffTreeType::MergeBaseWithWorktree { base } => vec![
                OsString::from("diff"),
                OsString::from("--raw"),
                OsString::from("-z"),
                OsString::from("--abbrev=64"),
            ...
            DiffTreeType::Since { base, head } => vec![
                OsString::from("diff-tree"),
                OsString::from("-r"),
                OsString::from("-z"),
                OsString::from("--abbrev=64"),
        ...
        self.executor
            .spawn(async move {
                ...
                let has_recreated_file = status.entries.iter().any(|(path, status)| {
                    matches!(
                        *status,
                        ...
                        tree_diff.entries.get(path),
                        Some(TreeDiffStatus::Deleted { .. })
                    )
                });
                if !has_recreated_file {
                    return Ok(tree_diff);
                }
                ...
                for (path, status) in status.entries.iter() {
                    ...
                    let Some(TreeDiffStatus::Deleted { old }) =
                        tree_diff.entries.get(path).cloned()
                    else {
                        continue;
                    };
