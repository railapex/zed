...
use crate::{
    ConflictSet, Project,
    git_store::{GitStoreEvent, Repository, RepositoryEvent, RepositorySnapshot, StatusEntry},
};
        ...
        while recv.next().await.is_some() {
            let Ok(()) = this.update(cx, |this, cx| {
                if this.repo.is_none() {
                    this.repo = this.project.upgrade().and_then(|project| {
                        project.read(cx).git_store().read(cx).active_repository()
                    });
                }
