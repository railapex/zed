...
pub struct GitStore {
    state: GitStoreState,
    project: Option<WeakEntity<crate::Project>>,
...
#[derive(Default)]
struct DisplayDiff {
    buffers: Option<Entity<diff_buffer_list::DiffBufferList>>,
    refresh: Option<Task<()>>,
...
impl GitStore {
    ...
    pub fn open_display_diff(
        ...
        buffer: Entity<Buffer>,
        cx: &mut Context<Self>,
    ) -> Task<Result<(Entity<BufferDiff>, bool)>> {
        let buffer_id = buffer.read(cx).remote_id();
        let (task, read_only) = match self.diff_base_for_buffer(buffer_id, cx) {
            Some((repo, oid)) => (self.open_diff_since(oid, buffer, repo, cx), true),
            None => (self.open_uncommitted_diff(buffer, cx), false),
        };
        cx.background_spawn(async move { Ok((task.await?, read_only)) })
    }
    ...
    pub fn display_repo_snapshots(&self, cx: &App) -> HashMap<RepositoryId, RepositorySnapshot> {
        ...
        for (id, snapshot) in &mut snapshots {
            if let Some(statuses) = self
                ...
                .and_then(|buffers| buffers.read(cx).statuses_by_path())
            {
    ...
    pub(crate) fn set_project(
        &mut self,
        project: WeakEntity<crate::Project>,
        cx: &mut Context<Self>,
    ) {
    ...
    pub fn diff_base_for_buffer(
        ...
        let buffers = self
            ...
            .as_ref()?
            .read(cx);
        let oid = match buffers.base_oid_for_path(&repo_path) {
            Some(oid) => oid,
            None if buffers
    ...
    pub fn display_diff_for_repo(
        ...
        self.display_diffs.get(&repo_id)?.buffers.clone()
    }
    ...
    pub fn ensure_display_diff(
        ...
        cx: &mut Context<Self>,
    ) -> Result<Entity<diff_buffer_list::DiffBufferList>> {
        let repo_id = repo.read(cx).id;
        if let Some(buffers) = self
            .display_diffs
            .get(&repo_id)
            .and_then(|state| state.buffers.clone())
        {
        ...
        let state = self.display_diffs.entry(repo_id).or_default();
    ...
    fn refresh_diff_base_for_repo(&mut self, repo_id: RepositoryId, cx: &mut Context<Self>) {
        if self.diff_base == GitDiffBaseSetting::Head && !self.display_diffs.contains_key(&repo_id)
        {
            return;
        }
...
mod tests {
    ...
    async fn test_merge_base_status_uses_worktree_contents(cx: &mut TestAppContext) {
        ...
        git_store.read_with(cx, |git_store, cx| {
            assert!(git_store.diff_base_for_buffer(buffer_id, cx).is_some());
